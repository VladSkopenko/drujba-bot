...
class AddressBook(UserList):
        ...
        if filtered_records:
            ...
            for record in filtered_records:
                serialize_record = {
                    ...
                    "Birthday": record.birthday.get_birthday.strftime("%Y-%m-%d")
                    if isinstance(record.birthday.get_birthday, date)
                    else None,
    ...
    def import_files(self, file):
        ...
        numbers1 = set(
            str(phone) for obj in source_data for phone in obj.get("Phones") or []
        )
        import_data = [
            obj
            for obj in import_data
            if not any(str(phone) in numbers1 for phone in obj.get("Phones") or [])
        ]
    ...
    def edit_phone(
        self, record: Record, serialize_record: {}, old_phone: str, new_phone: str
    ) -> Record:
        print()
            ...
            if self.user_info is not None:
                ...
                if email != "None" and password != "None":
                    receiver_email = [
                        ...
                        if item.email.get_email is not None
                    ]
