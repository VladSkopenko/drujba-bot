...
from FolderPath import (
    FOLDER_ACCOUNTS_PATH,
    FOLDER_NOTESBOOKS_PATH,
    FOLDER_ADDRESSBOOKS_PATH,
    create_folders,
...
class UserAccount:
    ...
    def register_account(self):  # Register Func
        console = Console()
        txt_files = [
            file for file in os.listdir(FOLDER_ACCOUNTS_PATH) if file.endswith(".txt")
        ]
        names = []
        ...
        if login in names:
            console.rule(
                title=f"[red]The username'{login}' is already taken.[/red]",
        ...
        console.rule(
            title=f"[green]Account'{self._user_name}' successfully created.[/green]",
            style="bright_magenta",
        )
    ...
    def descriptor(self, login, password):
        path = os.path.join(
            FOLDER_ACCOUNTS_PATH, f"{login}_account.txt"
        )  # Open Account Data
    ...
    def login(self):
        ...
        if (
            user_acc == False
            or login != user_acc._user_name
            or password != user_acc._user_password._password
        ):
            console.rule(
                title=f"[red]Invalid Login or Password. Try Again![/red]",
                style="bright_magenta",
            )
            return False
        else:
            console.rule(
                title=f"[green]{user_acc._user_name}, Welcome Back![/green]",
                style="bright_magenta",
            )
            time.sleep(3)
            return user_acc
    ...
    def testLogin(self):
        ...
        try:
            ...
            console.rule(
                title=f"[green]Login successful! The password and email are correct.[/green]",
                style="bright_magenta",
            )
            return True
        except imaplib.IMAP4.error as e:
            console.rule(
                title=f"[red]Error logging into the account![/red]",
                style="bright_magenta",
            )
            return False
        finally:
...
class UserPassword:
    ...
    def pass_ok(self):
        ...
        if password != repeat_password:
            console.rule(
                title=f"[red]Passwords do not match. Please try again.[/red]",
                style="bright_magenta",
            )
            return self.pass_ok()
...
class LoginCMD(cmd.Cmd):
    ...
    def do_login(self, *args):
        if self.userAccount.login():
            ...
            botcmd.book.user_info._account = os.path.join(
                FOLDER_ACCOUNTS_PATH, f"{botcmd.book.user_info._user_name}_account.txt"
            )
...
def start():
    ...
    table.add_column(
        "Welcome To Help Assistant", style="bright_magenta", width=100, justify="center"
    )
    create_folders()
    create_base_json_files()
