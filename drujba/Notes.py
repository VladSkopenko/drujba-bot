...
class Note_id(Field):  # ID нотатки
    ...
    def set_id(
        self, value
    ):  # setter перезаписує данні поля _id (Потреби в зміні id немає воно створюється при додаванні нотатки та є ідентефікатором)
...
class Title(Field):  # Титул
    ...
    def set_title(self, title: str):  # Перезаписує данні поля _title
        ...
        if title == None:  # Не може бути None
            raise ValueError(
                "Класс title, Метод set_title, в якості аргумента None -> очікується string: НЕКОРЕКТНІ ДАННІ "
            )
        if len(title) < 3 or len(title) > 10:  # Може містити від 3 до 10 символів
            raise ValueError(
                "Класс Title, Метод set_title() значення менше 3 або більше 10: НЕКОРЕКТНІ ДАННІ"
            )
        else:
            self._title = title
...
class Note(Field):  # Класс Нотатка
    ...
    def set_note(self, note):  # Перезаписує данні поля _note
        ...
        if note == None:  # Не може бути None
            raise ValueError(
                "Класс Note, Метод set_note, в якості аргумента None: НЕКОРЕКТНІ ДАННІ"
            )
        elif len(note) < 3:  # не може бути менше 3 символів
            raise ValueError(
                "Класс Note, Метод set_note, Нотатка має менше 3 символів: НЕКОРЕКТНІ ДАННІ"
            )
        else:
            self._note = note
...
class Tag(Field):  # Класс тег
    ...
    def set_tag(self, tag: str):
        ...
        if len(tag) == 0 or len(tag) > 1:  # один тег не може містити більше 1 слова
            raise ValueError(
                f"Класс Tag, Метод set_tag, тег складається з {tag} cлів очікується 1 слово: НЕКОРЕКТНІ ДАННІ"
            )
        elif len(tag[0]) < 3 or len(tag[0]) > 10:  # не може бути менше 3 і більше 10
            raise ValueError(
                f"Класс Tag, Метод set_tag, тег {tag[0]} має {len(tag[0])} букв очікується більше 3 і менше 10: НЕКОРЕКТНІ ДАННІ  "
            )
        else:
            self._tag = tag[0]
