...
class NotesBook(UserList):
    ...
    def add_note(self, input_title: str, input_note: str):
        calc_id = self.set_id()
        rec_note = NotesRecord(id=calc_id, note=input_note, title=input_title)
    ...
    def find_note(self, input_string: str):
        ...
        if input_string != "":
            for rec in self.data:
                if isinstance(rec.title.get_title, str) and re.search(
                    input_string.lower(), rec.title.get_title.lower()
                ):
                    find_list.append(rec) if not id(rec) in [
                        id(p) for p in find_list
                    ] else None
                if isinstance(rec.note.get_note, str) and re.search(
                    input_string.lower(), rec.note.get_note.lower()
                ):
                    find_list.append(rec) if not id(rec) in [
                        id(p) for p in find_list
                    ] else None
                if isinstance(rec.addition_date.get_date, str) and re.search(
                    input_string.lower(), rec.addition_date.get_date.lower()
                ):
                    find_list.append(rec) if not id(rec) in [
                        id(p) for p in find_list
                    ] else None
    ...
    def find_note_id(self, input_id: str):
        ...
        if input_id != "":
            for rec in self.data:
                if isinstance(rec.note_id.get_id, int) and re.search(
                    input_id.lower(), str(rec.note_id.get_id).lower()
                ):
                    find_list.append(rec) if not id(rec) in [
                        id(p) for p in find_list
                    ] else None
